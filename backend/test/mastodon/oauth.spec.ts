...
describe('Mastodon APIs', () => {
	describe('oauth', () => {
		...
		test('first login creates the user and redirects', async () => {
			...
			const actor = await db.prepare('SELECT * FROM actors').first<{ properties: string; email: string; id: string }>()
			...
			assert((await getSigningKey(userKEK, db, actor as unknown as Actor)) instanceof CryptoKey)
		})
